...
b4w.module["camera_anim"] = function(exports, require) {
   ...
   */
  exports.track_to_target = function(
    cam_obj,
    target,
    rot_speed,
    zoom_mult,
    ...
    if (!m_cam.is_eye_camera(cam_obj)) {
      m_print.error(
        "track_to_target(): Wrong camera object or camera move style"
      );
      return;
    }

    if (m_util.is_vector(target)) var obj_pos = target;
    else {
    ...
    var track_to_target_cb = function(obj, id, pulse) {
      ...
      if (pulse == 1) {
        ...
        if (curr_time < rot_time) {
          ...
          var smooth_coeff = smooth_function(
            curr_time - rot_time - zoom_time - zoom_delay / zoom_time
          );
          var delta_dist = (smooth_coeff * zoom_dist * elapsed) / zoom_time;
          m_trans.move_local(obj, 0, 0, delta_dist);
        } else {
  ...
  function init_limited_rotation_ratio(obj, limits, auto_rotate_ratio) {
    ...
    return (
      auto_rotate_ratio *
      Math.min(1, delts[0] / ROTATION_OFFSET, delts[1] / ROTATION_OFFSET)
    );
  }
  ...
  function get_delta_to_limits(obj, angle, limit_left, limit_right, dest) {
    ...
    if (
      Math.abs(delta_to_left) < ROTATION_LIMITS_EPS ||
      2 * Math.PI - Math.abs(delta_to_left) < ROTATION_LIMITS_EPS
    )
      delta_to_left = 0;
    if (
      Math.abs(delta_to_right) < ROTATION_LIMITS_EPS ||
  ...
  exports.auto_rotate = function(
    ...
    function update_limited_rotation_params(curr_limits) {
      ...
      cur_rotate_ratio = init_limited_rotation_ratio(
        obj,
        angle_limits,
        auto_rotate_ratio
      );
    ...
    function elapsed_cb(obj, id, pulse) {
      if (pulse == 1) {
        ...
        if (move_style == m_cam.MS_STATIC) disable_cb();
          ...
          if (
            angle_limits === null ||
            curr_limits.left != angle_limits.left ||
    ...
    function limited_auto_rotate(obj, id) {
      ...
      if (delts[1] > rot_offset && delts[0] > rot_offset)
    ...
    if (!m_ctl.check_sensor_manifold(obj, "AUTO_ROTATE")) {
      ...
      else var wheel_zoom = m_ctl.create_custom_sensor(0);

      var logic_func = function(s) {
        return (s[0] && s[2]) || (s[1] && s[2]) || s[3] || s[4] || s[5];
      };
  ...
  exports.check_auto_rotate = function() {
    ...
    if (cam_type == m_cam.MS_STATIC) return false;
   ...
   */
  exports.move_camera_to_point = function(
    cam_obj,
    point_obj,
    cam_lin_speed,
    cam_angle_speed,
    ...
    if (!cam_obj) {
      m_print.error(
        "move_camera_to_point(): you must specify the camera object"
      );
      return;
    }
    ...
    if (!point_obj) {
      m_print.error(
        "move_camera_to_point(): you must specify the point object"
      );
      return;
    }
    ...
    if (m_util.is_vector(cam_obj)) var cam_tsr = cam_obj;
    else var cam_tsr = m_trans.get_tsr(cam_obj, _tsr_tmp);

    if (m_util.is_vector(point_obj)) var point_tsr = point_obj;
    else var point_tsr = m_trans.get_tsr(point_obj, _tsr_tmp2);

    var distance = m_vec3.distance(
      m_tsr.get_trans_view(cam_tsr),
      m_tsr.get_trans_view(point_tsr)
    );
    var move_time = distance / cam_lin_speed;

    var current_cam_dir = m_util.quat_to_dir(
      m_tsr.get_quat_view(cam_tsr),
      m_util.AXIS_MZ,
      _vec3_tmp
    );
    var target_cam_dir = m_util.quat_to_dir(
      m_tsr.get_quat_view(point_tsr),
      m_util.AXIS_MZ,
      _vec3_tmp2
    );

    var vec_dot = Math.min(
      Math.abs(m_vec3.dot(current_cam_dir, target_cam_dir)),
      1
    );
  ...
  exports.rotate_camera = function(cam_obj, angle_phi, angle_theta, time, cb) {
    ...
    var cur_animator = m_time.animate(0, angle, time, function(e) {
      ...
      else if (angle_theta) m_cam.rotate_camera(cam_obj, 0, delta_theta);
      else if (angle_phi) m_cam.rotate_camera(cam_obj, delta_phi, 0);
