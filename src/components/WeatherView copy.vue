...
export default {
  ...
  computed: {
    ...
    deadline: vm => new Date(vm.weather.dt_txt).getTime()
  },
  ...
  methods: {
    onReplace() {
      this.isTimer = false;
      this.$store.dispatch("initWeatherData").then(status => {
        if (status) this.checkDeadline();
      });
    },
    ...
    checkDeadline() {
      ...
      let nowUtcTime = nowUtc.getTime();
      this.isTimer = this.deadline > nowUtcTime;
      return nowUtcTime;
    },
...
<template>
  <div>
    <Countdown
      v-if="deadline > checkDeadline() && isTimer"
      :deadline="deadline"
      @timeEnd="onReplace"
      :view="false"
    />
